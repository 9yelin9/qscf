...
class VQE:
    ...
    def circuit_gamma(self, theta, basis):
        ...
        #return [qml.expval(p) for p in self.projector]
    ...
    def get_dm(self, theta, shots):
        gamma = np.array([self.qnode_gamma(theta, basis, shots=shots) for basis in self.bases[:self.Nocc]], dtype=float).reshape(-1, self.N, self.N)
    ...
    def cost_qdft_old(self, theta, fock, hamiltonian, alpha, shots):
    ...
    def init_dm(self, theta, shots, max_iter=100, tol=1e-1):
        ...
        return pnp.array(res.x, dtype=float, requires_grad=True), dm_target
        """
        ...
        for i in range(max_iter):
            ...
            theta, norm2 = opt.step_and_cost(self.cost_dm,
                                             theta, dm_target=dm_target, shots=shots,
    ...
    def qdft_old(self, theta, shots=None, max_iter=1000, e_tol=1e-6, mix=0.3, alpha=10):
        ...
        for itr in range(max_iter):
            ...
            res = scipy.optimize.minimize(self.cost_qdft_old, theta,
        ...
        with open(f'{self.output_dir}/qdft_old.etot', 'w') as f: f.write(f'{etot:f}')
        tm, ts = divmod(int(time.time() - t0), 60)
        print(f'{self.qdft_old.__name__} time elapsed: {tm}m {ts}s', end='\n\n')

    def qdft(self, theta, shots=None, max_iter=1000, tol=1e-3):
        ...
        res = scipy.optimize.minimize(self.cost_qdft, theta,
                                      ...
                                      options={'disp': True})
        theta = res.x
        etot = self.cost_qdft(theta, shots) 
        ...
        for i in range(max_iter):
            etot_old = etot
            theta, etot = opt.step_and_cost(self.cost_qdft,
                                            theta, shots=shots,
                                            nums_frequency={'theta': {(i,): 1 for i in range(len(theta))}})
...
theta = 2*np.pi * pnp.random.rand(vqe.M * (vqe.L+1), requires_grad=True) # for qml.optimizer
#vqe.dft()
#vqe.dft_custom()
#vqe.qdft_old(theta)
vqe.qdft(theta)
